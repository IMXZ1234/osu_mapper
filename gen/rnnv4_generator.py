...
class RNNGenerator(BeatmapGenerator):
    def __init__(self,
    ...
    def generate_beatmapset(self, audio_file_path, speed_stars_list, meta_list,
        ...
        for beatmap in beatmap_list:
            ...
            last_label = gen.init_last_label()
            beatmap_labels = []
            ...
            # # print('warmup_len')
            # # print(warmup_len)
            # # period = [0] * (2 * self.snap_divisor)
            # # period[0] = 2
            # # period[-1] = 1
            # warmup_labels = [0] * (warmup_len - len(period)) + period
            ...
            for label_idx in range(len(gen)):
                ...
                last_label, state = self.inference.run_inference_sample_rnn(feature, state)
                beatmap_labels = np.concatenate([beatmap_labels, last_label])
            beatmap_label_list.append(beatmap_labels)
        print(beatmap_label_list[0].tolist())
