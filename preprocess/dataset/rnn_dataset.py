...
class RNNDataset(fit_dataset.FitDataset):
    ...
    def make_feature(self, mel, label):
        """
    ...
    class RNNSampleDataGenerator:
        ...
        def get_next_sample_feature(self, last_label, warmup=False):
            ...
            # print(feature)
            return (feature,
                    self.beatmap.bpm_min() / self.dataset.coeff_speed_stars,
                    self.beatmap.speed_stars() / self.dataset.coeff_bpm)
    ...
    def prepare(self, save=True):
        ...
        for id_ in ids:
            ...
            # crop_start_time is also aligned with snaps
            # print('cal first_ho_snap before round')
            # print((first_ho_time - crop_start_time) / snap_ms)
            ...
            data.append([sample_data_list, speed_stars / self.coeff_speed_stars, beatmap.bpm_min() / self.coeff_bpm])
            label.append(np.asarray(beatmap_label))

        # max_value = np.max([np.max(seq_data[0]) for seq_data in data])
        # for idx in range(len(data)):
        #     data[idx][0][:-label_num] = data[idx][0][:-label_num] / max_value
