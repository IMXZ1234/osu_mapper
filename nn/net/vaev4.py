...
class VAE(nn.Module):
    """try to predict a beat each step"""
    def __init__(self, in_channels, cond_data_feature_dim, hidden_dim,
                 step_snaps=8,
                 pos_emb_period=12, pos_emb_channels=4,
        ...
        # encoder
        self.gru_enc = nn.GRU(step_snaps * (cond_data_feature_dim + in_channels) + pos_emb_channels, hidden_dim, bidirectional=True, num_layers=enc_layers)
        self.z_dim = h_dim
        ...
        self.gru_dec = nn.GRU(step_snaps * (cond_data_feature_dim + in_channels) + pos_emb_channels, hidden_dim, bidirectional=False, num_layers=dec_layers)
        # self.fc_dec1 = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.fc_dec = nn.Linear(self.hidden_dim, step_snaps * in_channels)
    ...
    def encode(self, cond_data, label, pos_emb):
        batch_size, step_num, _ = cond_data.shape
    ...
    def decode(self, z, cond_data, pos_emb, label=None, teacher_forcing_ratio=0.5):
        batch_size, step_num, _ = cond_data.shape
        h = self.fc_z(z).reshape([batch_size, self.dec_layers, -1]).permute(1, 0, 2).contiguous()
        all_out = []

        out = torch.ones([batch_size, self.in_channels * self.step_snaps], dtype=torch.float, device=cond_data.device) / 2
        for i in range(step_num):
            gru_input = torch.cat([cond_data[:, i], out, pos_emb[:, i]], dim=1).unsqueeze(dim=0)
            out, h = self.gru_dec(gru_input, h)
            out = out.reshape([batch_size, -1])
            # out = self.fc_dec(F.relu(self.fc_dec1(out)))
            out = self.fc_dec(out)
            all_out.append(out)
            if label is not None and random.random() < teacher_forcing_ratio:
                out = label[:, i]
        recon = torch.stack(all_out, dim=1).reshape([batch_size, step_num * self.step_snaps, -1])
        return recon, h
    ...
    def forward(self, cond_data, label, teacher_forcing_ratio=0.5):
        """
        label: [batch_size, seq_len, in_channels=4(circle_density, slider_density, x, y)]
        cond_data: [batch_size, seq_len, cond_data_feature_dim]
        """
        batch_size, seq_len, feature_num = cond_data.shape
        cond_data = cond_data.reshape([batch_size, seq_len // self.step_snaps, -1])
        label = label.reshape([batch_size, seq_len // self.step_snaps, -1])
        batch_size, step_num, _ = cond_data.shape

        pos_idx = gen_pos_seq(batch_size, step_num, self.pos_emb_period, cond_data.device)
