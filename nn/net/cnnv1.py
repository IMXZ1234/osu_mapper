...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        self.cnn_layers = nn.Sequential(
            nn.BatchNorm1d(2),
            nn.Conv1d(2, 64, kernel_size=9, stride=1, padding=4),
            ...
            nn.ReLU(inplace=True),
            # nn.BatchNorm1d(256),
            # nn.Conv1d(256, 512, kernel_size=3, stride=4, padding=1),
            # nn.ReLU(inplace=True)
        )
        # last feature is speed_stars, which serves as an 'offset'
        self.fcn = nn.Conv1d(256 + 1, 2, kernel_size=1, stride=1, padding=0)
