...
class ResNet1D(nn.Module):
    ...
    def __init__(self, num_block=(1, 1, 1, 1), num_classes=2, n_mel=128, block='BasicBlock'):
        super().__init__()
        ...
        self.in_channels = n_mel

        self.conv1 = nn.Sequential(
            nn.Conv1d(self.in_channels, 64, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        x = self.fc(x).permute([0, 2, 1]).squeeze(dim=0)
