...
class ResNet1D(nn.Module):
    ...
    def __init__(self, num_block=(1, 1, 1, 1), num_classes=2, block='BasicBlock'):
        super().__init__()
        ...
        self.conv4_x = self._make_layer(block, 256, num_block[2], 4)
        self.conv5_x = self._make_layer(block, 512, num_block[3], 4)
        # self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.fc = nn.Conv1d(512 * block.expansion + 1, num_classes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        x, speed_stars, valid_interval = x

        for i in range(len(x)):
            x[i] = x[i].unsqueeze(dim=0)
            x[i] = self.conv1(x[i])
            x[i] = self.conv2_x(x[i])
            x[i] = self.conv3_x(x[i])
            x[i] = self.conv4_x(x[i])
            x[i] = self.conv5_x(x[i])
            # x[i] = self.avg_pool(x[i])
            # x[i] = x[i].reshape([1, -1])
            # [N, C, snap_num]
            speed_stars_feature = torch.tensor(speed_stars[i],
                                               device=x[i].device,
                                               dtype=torch.float).expand([1, 1, x[i].shape[2]])
            # cat at channel dim
            x[i] = torch.cat([x[i], speed_stars_feature], dim=1)
            x[i] = self.fc(x[i]).permute([0, 2, 1]).squeeze(dim=0)
