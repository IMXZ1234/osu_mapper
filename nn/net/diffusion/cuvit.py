...
class CUViT(nn.Module):
    def __init__(
        ...
        self.meta_mlp = nn.ModuleList([nn.Sequential(
            sinu_pos_emb,
            nn.Linear(fourier_dim, time_dim),
            nn.GELU(),
            nn.Linear(time_dim, time_dim)
    ...
    def forward(self, x, time, cond_data):
        audio, meta = cond_data
        audio_embed = self.audio_encoder(audio)
        meta_embed = [
            me(m)
            for m, me in zip(meta, self.meta_mlp)
        ]
