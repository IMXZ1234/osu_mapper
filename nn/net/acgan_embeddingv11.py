...
class Generator(nn.Module):
    ...
    def forward(self, cond_data, noise=None):
        ...
        coord_output = self.decoder_coord(coord_x) + coord_x
        coord_output = self.decoder_coord_final(coord_output)
        embed_output = self.decoder_embedding(embed_x) + embed_x
    ...
    def __init__(self, n_snap, tgt_embedding_dim=16, tgt_coord_dim=2, audio_feature_dim=40, norm='LN', middle_dim=256, audio_preprocess_dim=24,
        ...
        self.tgt_embedding_preprocess = nn.Sequential(
            CNNExtractor(
                ...
                n_beats,
                stride_list=(1, 1, 1, 1),
                out_channels_list=(middle_dim, middle_dim, middle_dim, preprocess_dim),
