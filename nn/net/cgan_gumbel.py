...
class Generator(nn.Module):
    ...
    def forward(self, cond_data, type_label, ho_pos, hidden, pos):
        ...
        type_label_out = self.gru2out_label(out)       # batch_size x vocab_size
    ...
    def sample(self, cond_data, h=None, start_letter=0, start_ho_pos=(0.5, 0.5), valid_len=None):
        ...
        type_label = F.one_hot(type_label, num_classes=self.vocab_size).float()
        ...
        for i in range(seq_len):
            ...
            # num_samples x 3, num_samples x 2
            (type_label_out, ho_pos_out), hidden = self.forward(cond_data[:, i], type_label, ho_pos, h, pos)               # out: num_samples x vocab_size
            type_label = type_label_out
            type_label_out = F.gumbel_softmax(type_label_out, dim=1, hard=True)  # num_samples x vacab_size
            type_label_samples.append(type_label_out.squeeze(dim=1))
            ho_pos_samples.append(ho_pos_out)

            ho_pos = ho_pos_out
        # -> num_samples x seq_len x 2
        ho_pos_samples = torch.stack(ho_pos_samples, dim=1)
        type_label_samples = torch.stack(type_label_samples, dim=1)
    ...
    def batchNLLLoss(self, cond_data, inp, target, h=None):
        ...
        for i in range(seq_len):
            ...
            type_label_inp = F.one_hot(type_label[i], num_classes=self.vocab_size).float()
            (type_label_out, ho_pos_out), h = self.forward(cond_data[i], type_label_inp, ho_pos[i], h, pos)
...
class Discriminator(nn.Module):
    ...
    def forward(self, cond_data, inp, h):
        """
        inp: batch_size x seq_len x 3 / [type_label, ho_pos]: batch_size x seq_len x vocab_size, batch_size x seq_len x 2
        """
        type_label, ho_pos = inp
        type_label = type_label.permute(1, 0, 2)
        ho_pos = ho_pos.permute(1, 0, 2)
        total_len, batch_size, _ = type_label.shape
        cond_data = cond_data.permute(1, 0, 2)
        # 1 x total_len -> batch_size, total_len
        pos = (torch.arange(total_len, device=cond_data.device, dtype=torch.long) % self.seq_len).reshape([total_len, 1]).expand([total_len, batch_size])
    ...
    def batchClassify(self, cond_data, inp, h=None):
        ...
        if not isinstance(inp, (list, tuple)):
            inp = (F.one_hot(inp[:, :, 0].long(), num_classes=self.vocab_size).float(), inp[:, :, 1:])
        if h is None:
