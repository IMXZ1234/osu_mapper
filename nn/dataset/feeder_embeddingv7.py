...
class SubseqFeeder(torch.utils.data.Dataset):
    ...
    def load_subseq(self, subseq_idx):
        ...
        with open(meta_path, 'rb') as f:
            star, cs, ar, od, hp, bpm = pickle.load(f)
        # [mel_frame, n_mel=40]
        ...
        data = data[start * self.beat_divisor * self.mel_frame_per_snap: min(len(data), end * self.beat_divisor * self.mel_frame_per_snap)]
        if pad_beats > 0:
        ...
        meta = np.array([star / 10, (bpm - 50) / 360])
        # print('meta', meta)
        ...
        if not self.inference:
            ...
            if 'embedding' in self.item:
                with open(label_idx_path, 'rb') as f:
                    # n_beats, self.mel_frame_per_snap
