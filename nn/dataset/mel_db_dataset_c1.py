...
from nn.dataset.dataset_util import hitobjects_to_label, hitobjects_to_density
from util import beatmap_util
from preprocess import db


class MelDBDatasetC1(Dataset):
    """
    ...
    def __init__(self, db_path, audio_dir,
        ...
        """
        super(MelDBDatasetC1, self).__init__()
        ...
        self.start_time_list = [beatmap_util.get_first_hit_object_time_microseconds(beatmap) for beatmap in
                                self.beatmaps]
        self.end_time_list = [beatmap_util.get_last_hit_object_time_microseconds(beatmap) for beatmap in self.beatmaps]

        self.audio_snap_num = [MelDBDatasetC1.cal_snap_num(bpm, start_time, end_time, self.snap_divisor)
        ...
        self.audio_start_end_mel = [
            MelDBDatasetC1.get_aligned_start_end_mel_frame(
                self.bpm_list[idx],
        ...
        if not self.inference:
            ...
            if not density_label:
                ...
                self.audio_label = [hitobjects_to_density(beatmap,
                                                          start_time,  # first hit object time
                                                          snap_per_microsecond,
                                                          audio_snap_num,
                                                          self.sample_snaps, )
    ...
    def __getitem__(self, index):
        ...
        if cached is not None:
            ...
            preprocessed_audio_data = MelDBDatasetC1.preprocess(audio_data,
