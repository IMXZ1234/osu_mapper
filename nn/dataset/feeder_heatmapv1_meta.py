...
class SubseqFeeder(torch.utils.data.Dataset):
    ...
    def load_subseq(self, subseq_idx):
        ...
        if pad_len > 0:
            cat = np.pad(cat, ((0, pad_len * self.coeff_data_len), (0, 0)), mode='reflect')
            if not self.pad:
                print('padding occur!')
        cat = cat[start * self.coeff_data_len: end * self.coeff_data_len]
        data, data_dis_dim, meta_pred = cat[:data_dim], cat[data_dim: data_dis_dim + data_dim], cat[-meta_pred_dim:]
        ...
        if not self.inference:
            ...
            if pad_len > 0:
                if not self.pad:
                    print('padding occur!')
                label = np.pad(label, ((0, pad_len * self.coeff_label_len), (0, 0)), mode='reflect')
        ...
        #     print(np.max(data), np.min(data))
        return data, data_dis, meta_pred, label

    def __len__(self):
        return len(self.subseq_dict)

    def __getitem__(self, index):
        data, data_dis, meta_pred, label = self.load_subseq(index)
        if self.inference:
            return data, index
        else:
            return data, data_dis, meta_pred, label, index
