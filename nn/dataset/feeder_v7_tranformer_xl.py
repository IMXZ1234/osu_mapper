...
class Feeder(torch.utils.data.Dataset):
    ...
    def load_beatmap(self, index):
        ...
        ho_type = np.pad(ho_type, ((0, pad_beats * self.beat_divisor + 1),), mode='constant')
        ...
        assert len(ho_type) == tgt_snaps + 1
        out_ho_type = ho_type[1:]
        out_ho_type = out_ho_type.reshape([tgt_steps, self.subseq_snaps]).T
        inp_ho_type = ho_type[:-1]
        inp_ho_type = inp_ho_type.reshape([tgt_steps, self.subseq_snaps]).T
